...
var runTimeseriesSyncSSH = ssh.RunWithTimeout
...
func syncJobTimeseries(database *sql.DB, job *db.Job, timeout time.Duration) error {
	...
	if err != nil {
		return fmt.Errorf("refresh job before timeseries sync: %w", err)
	}
	if current == nil {
		return nil
	}
	...
	stdout, _, err := runTimeseriesSyncSSH(job.Host, cmd, timeout)
