...
func initSchema(db *sql.DB) error {
	...
	if _, err := db.Exec(`
		CREATE TABLE IF NOT EXISTS autopilot_state (
			...
			active_binary_path       TEXT,
			active_binary_size       INTEGER,
			active_binary_mtime      INTEGER,
			active_binary_dev        INTEGER,
			active_binary_ino        INTEGER,
	...
	for _, stmt := range []string{
		...
		`ALTER TABLE job_runs ADD COLUMN error_diagnosis_backfilled INTEGER NOT NULL DEFAULT 0`,
