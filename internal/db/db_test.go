...
func TestListActiveJobsExcludesLaunchOwnedHostCorruption(t *testing.T) {
	...
	if _, err := database.Exec(
		...
		t.Fatal("seed corrupt mixed target succeeded; want invariant failure")
	}
}
...
func TestJobTargetKindAndDisplay(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:          "legacy rental host alias still reads as rental",
			job:           &Job{Host: "rental:17", Status: StatusQueued},
			wantKind:      JobTargetRentalInstance,
			wantDisplay:   "wi17",
			wantRental:    true,
			wantInventory: false,
...
// TestOpen_RunsMigrationsAfterSchemaVersionBump regresses the case where a
// migration is added but currentSchemaVersion is not bumped: an existing DB
// at the old version hits the user_version fast-path in initSchema and the
// new ALTER TABLE never runs, so subsequent queries against the new column
// fail with "no such column". This test simulates that exact upgrade path
// for the max_compute_cap column.
func TestOpen_RunsMigrationsAfterSchemaVersionBump(t *testing.T) {
	tmpFile, err := os.CreateTemp("", "weft-schema-bump-*.db")
	if err != nil {
		t.Fatalf("create temp file: %v", err)
	}
	tmpFile.Close()
	...
	// First Open builds the schema at currentSchemaVersion.
	database, err := Open()
	if err != nil {
	...
	// Roll user_version back to the prior schema version. If a future
	// migration is added without bumping currentSchemaVersion, the fast-path
	// (version >= currentSchemaVersion) will short-circuit and the migration
	// will not re-run on this "old" DB.
	...
	// Second Open should re-run migrations and restore max_compute_cap.
	database, err = Open()
	if err != nil {
