...
// RestoreNoStartMoveTargetToSource restores a move-to-new job to its live
// source launch when its target launch failed before the job started. It is
// idempotent so background repair paths can consume stale open intents whose
// target attempt was already closed.
func RestoreNoStartMoveTargetToSource(database *sql.DB, jobID, targetLaunchID int64, outcome string) (bool, error) {
	tx, err := database.Begin()
	if err != nil {
		return false, err
	}
	now := time.Now().Unix()
	restored, err := restoreNoStartMoveToSourceTx(tx, jobID, targetLaunchID, now, outcome)
	if err != nil {
		tx.Rollback()
		return false, err
	}
