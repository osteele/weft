...
func buildSearchFilter(c OfferConstraints) (string, func([]Offer) []Offer) {
	...
	if c.MinGPUMemGB > 0 {
		// MinGPUMemGB is already an effective placement floor. Add no further
		// headroom here; doing so double-pads explicit reservations persisted by
		// submission. Retain the historical hardware-ceiling rollback so older
		// jobs that stored 82GB for an A100 80GB can replan automatically.
		// Filter uses GB (response field is MB).
		parts = append(parts, fmt.Sprintf("gpu_ram>=%d", StoredMemFloorGB(c.GPUClass, c.MinGPUMemGB)))
	}
	if c.MinDiskGB > 0 {
