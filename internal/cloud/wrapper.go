...
// GenerateAgentWrapper produces a thin bash script (~30 lines) that delegates
// job execution to the weft-agent binary. The agent handles process management,
// telemetry sampling, failure detection, and completion records. The wrapper
// handles only R2 upload and self-destruct.
func GenerateAgentWrapper(client Client, jobs []AgentJob, r2Bucket string) string {
	...
	for _, job := range jobs {
		...
		b.WriteString(fmt.Sprintf("rclone copy $LOG_DIR/ \"r2:$R2_BUCKET/jobs/$JOB_ID/results/\" 2>/dev/null\n"))
