...
var constraintSystemEnforcement = map[PlacementSystem]map[string]systemCoverageNote{
	...
	SystemOnPrem: {
		...
		"cpu_cores":     {checkedInSystem, "scoreHost explicit floor; the cpu-intensive tag floor stays cloud-only by design (it avoids paying for weak rentals; on-prem it remains a scoring preference)"},
		"host_ram":      {checkedInSystem, "scoreHost RAM floor (unknown inventory passes)"},
		"interconnect":  {checkedInSystem, "scoreHost via placement.InterconnectSatisfied over host GPU naming"},
