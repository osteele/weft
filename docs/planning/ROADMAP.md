...
| Skip past gated benchmarks | Medium | When a benchmark-isolation-tagged job at the head of the queue is gated on host CPU/RAM thresholds (`internal/runner/benchmark.go`), the runner currently blocks the entire queue waiting for the host to go idle — which it may never do on a shared dev machine. Instead, read past the gated benchmark to the next non-benchmark job and start that, while keeping the benchmark in pending. Triggered today: wj557 sat in cool30's pending list with `benchmark.waiting cpu=16%>5%` for 30+ minutes, blocking ~28 other ready jobs. Care needed: don't promote a non-benchmark job whose start would itself push CPU above the threshold (would just defer the benchmark forever); fall back to "wait" if no eligible non-benchmark job exists. | — |
...
| Unify constraint resolution | Medium | Placement constraints are resolved by three divergent paths: submit-time persistence (`cmd/run.go`), placement-time re-derivation (`placement.ConstraintsFromJob`), and the reuse path's inline checks (`campaign.MatchJobToInstanceWithUV`, which validates only GPU class/memory and skips CUDA/driver floors). The lazy `max_compute_cap` backfill adds a timing seam where the persisted and re-derived values can diverge silently. Extract one shared resolution path used by all three, validate persisted-vs-derived consistency, and retire the inline reuse checks. See `docs/architecture/placement.md` § Known debt. | — |
...
## Migration test no longer forces per-migration idempotency

`TestOpen_TakesMigrationBackupOnUpgrade` now models a real upgrade by
using a database whose schema and goose version are both at
`Target() - 1`. The test verifies that a backup is taken before the
pending migration runs, without forcing every numbered migration to
re-execute against an already-current schema.

Version 12 is therefore a normal goose SQL migration again. Version 9
still uses a Go migration because the squashed pre-goose baseline already
contains `launches.first_onstart_probe_seen_unix`; that is a separate
compatibility path for the one existing pre-goose database, not a general
requirement for future `ALTER TABLE ... ADD COLUMN` migrations.
