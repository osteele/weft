...
│   ├── campaign/          # Internal cloud batch engine (launch, cost estimation, formatting)
...
│   ├── ui/                # Terminal and dashboard UI packages
...
Ephemeral Vast.ai instances run the same Go agent binary as persistent hosts,
with `weft-agent run-instance` coordinating instance-local setup, telemetry, and
job execution. The cloud launcher deploys the agent binary and rsyncs project
sources via SSH, then starts a thin shell wrapper (~30 lines) that:

1. Invokes the agent job loop for each assigned job
2. Uploads per-job results (logs, completion record, timeseries, phases) to R2
3. Writes a batch completion marker to R2
