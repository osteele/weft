...
`SetupSurvival.TerminateAfter` when the setup survival curve has learned one;
the constants `onStartStallTimeout` (10 min) and `onStartTotalActiveTimeout`
(25 min) survive as fallbacks.

As first written the preference was gated on `TerminateAfter > 0`, which is
never false: `ComputeSetupSurvival` and `ComputeBootstrapSurvival` return a
populated struct carrying *default* thresholds when history is insufficient,
flagging that with `TerminateLearned` rather than a zero value. Every watchdog
...
unreachable. That was a defect, not a decision, and it is fixed: the gate is
now `TerminateLearned`. This record concerns only the behavior that remains —
what happens when a threshold genuinely was learned.
...
1. **Both windows collapse to one duration.** With learned data, the stall
   ...
   `SetupSurvival.TerminateAfter`; they differ only in anchor (last marker
...
3. **The learning is blind to the failure mode it now governs.** Setup
   ...
   incidents the watchdog exists to catch. The `TerminateLearned` gate bounds
   this: a project with no setup history keeps the 25-minute constant, so the
...
- **The exposure is bounded and small in dollars.** The worst case adds
  ...
  incident).
- **A threshold learned high is evidence, not noise.** `TerminateAfter`
  exceeds 25 min only when historical setups for that command/workdir
