...
func TestApplyGPUMemHeadroom(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := applyGPUMemHeadroom(tt.memGB, tt.hasExplicit, tt.strict, tt.hardware); got != tt.want {
				t.Fatalf("applyGPUMemHeadroom(%d, explicit=%v, strict=%v, hardware=%v) = %d, want %d",
					tt.memGB, tt.hasExplicit, tt.strict, tt.hardware, got, tt.want)
			}
		})
	}
}
